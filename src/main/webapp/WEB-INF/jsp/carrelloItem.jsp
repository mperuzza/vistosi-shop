...
<form:form modelAttribute="carrelloItem" method="post" action="${formURL}" id="carrelloItemForm${isRicambio?'Ric':''}">
    ...
    <security:authorize ifNotGranted="ROLE_ANONYMOUS">
        ...
        <div id="prezzi" class="dd-scheda clearfix det-row">
            <div id="prz-lis" class="det-block <c:if test="${carrelloItem.articolo.fgpromo=='S'}">offerta</c:if>"><span class="det-label"><span style="text-transform:capitalize"><spring:message code="price.lis" text="Prezzo di listino"/></span></span><span class="det-value"><fmt:formatNumber value="${prezzo}" pattern="#,##0.00"/> ${valuta}</span></div>
            ...
            <div id="prz-net" class="det-block"><c:if test="${cookie.view_net_price.value == 'S' && !hideCond}"><span class="det-label"><span style="text-transform:capitalize"><spring:message code="price.net" text="Prezzo netto"/></span></span><span class="det-value"><fmt:formatNumber value="${prezzo_netto}" pattern="#,##0.00"/> ${valuta}</span></c:if></div>
        </div>
