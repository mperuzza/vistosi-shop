...
<div id="head-user">
        ...
        <% }%>    
        <div id="user"><security:authorize ifAnyGranted="ROLE_ADMIN, ROLE_CAPO, ROLE_AGEN,ROLE_ISPE,ROLE_GEST" ifAllGranted="ROLE_CLIE"> <spring:message code="cliente" text="Cliente"/>: <a href="<c:url value="/changeClie"/>" title="<spring:message code="click.change.cliente" text="Clicca per cambiare cliente"/>"><security:authentication property="principal.cliente.archenti.ragcog"/></a> / </security:authorize> <security:authorize ifNotGranted="ROLE_ANONYMOUS"><spring:message code="user" text="Utente"/>: <strong><security:authentication property="principal.userDB.dsutente"/></strong></security:authorize>
    </div>
</div>

<div id="head-logo-cart" class="clearfix">
    <div id="logo">
        <a href="<c:url value="/index"/>">vistosi</a>
    </div>    
...
<div id="btn_menu" class="clearfix">
    ...
    <ul>
        ...
        <li class="bt_ricerca"><a href="<c:url value="/findArticoli"/>"><spring:message code="menu.ricerca" text="Ricerca avanzata"/></a></li>
        ...
        <li class="bt_speciali"><a href="/portal/epCat_comgen_art_spec_edit.jsp" rel="popup[800,500,r]"><spring:message code="menu.artspeciali" text="Articoli speciali"/></a></li>
    ...
    <div id="vshop_menu">
        <ul>
            ...
            <li id="m-collezione" class="ddmenu<c:if test="${disableMenu}"> disabled </c:if>">
                ...
                <c:if test="${rc.locale!='it'}"><c:set var="dsvistcol">dsvistccol_${rc.locale}</c:set></c:if>
            ...
            <li id="m-famiglia" class="ddmenu<c:if test="${disableMenu}"> disabled </c:if>">
                ...
                <c:if test="${!disableMenu}"><div class="m-list-cnt">
                ...
                <div class="list-wrapper"><ul></c:if>
                    <li class="column${colnum} menu_item <c:if test='${s.index%blockColumn == 0}'>coltopper</c:if><c:if test='${f.cdvistfam==famigliaFilter.cdvistfam}'> active</c:if>" >               
                        ...
                        <a href="${baseURL}" <c:if test="${!empty f.stato}">style="color: rgb(${f.stato.vist_rgb_r},${f.stato.vist_rgb_g},${f.stato.vist_rgb_b});"</c:if>>${ep:capitalizeFirst(fn:toLowerCase(f[dsfam]))}</a>
                    </li>
        ...
        <table>
            ...
            <tr>
                ...
                <td class="m-famiglia"><c:if test="${!empty famigliaFilter && !plainMenu}">
                        ...
                        <a href="${clearURL}" class="delete-filter"><c:out value="${fn:toLowerCase(famigliaFilter[dsfam])}"/></a></c:if></td>
...
<div id="stati-cnt" class="clearfix">
    <div style="float: left;">Filtra per:</div>
    <ul><c:set var="dsstato">dsstato</c:set>
        <c:if test="${rc.locale!='it'}"><c:set var="dsstato">dsstato_${rc.locale}</c:set></c:if>
    ...
    <div class="button right logout" id="reset_filtri" style="float: right;"><span><em><a href="<c:url value="/index"/>"><spring:message code="filter.reset" text="Azzera tutti i filtri"/></a></em></span></div>
