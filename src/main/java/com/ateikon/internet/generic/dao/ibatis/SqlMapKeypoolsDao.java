...
public class SqlMapKeypoolsDao extends SqlMapBaseDao {
    ...
    public long getNextId(String tabella, BaseTableBean btb) throws DataAccessException {
        ...
        String cdagen_const = getCdagenConst(getProfil().getCdazie());
        /* check key exists */
        //if(!isKey(btb.getCddipa(), cdagen_const, cdprog)) return -1;
        if(!isKey(getProfil().getCddipa(), cdagen_const, cdprog)) return -1;
        
        //Keypools keypools = new Keypools(cdprog, btb.getCddipa(), cdagen_const);
        Keypools keypools = new Keypools(cdprog, getProfil().getCddipa(), cdagen_const);
        ...
        if (ll_key == ll_key1){
            ...
            next.setCddipa(getProfil().getCddipa());
   ...
   public long getNextIdGest(String tabella, BaseTableBean btb) throws DataAccessException {
        ...
        if(!isKeyGest(getProfil().getCddipa(), getProfil().getCdazie(), cdprog)) return -1;
        
        //Keypools keypools = new Keypools(cdprog, btb.getCddipa(), btb.getCdazie(), null);
        Keypools keypools = new Keypools(cdprog, getProfil().getCddipa(), getProfil().getCdazie(), null);
        keypools = getKeypools(keypools);
        log.debug("-- " + keypools);
        ...
        Keypools next = new Keypools(cdprog, null, getProfil().getCdazie(), null);
        // Ritorno la chiave valida se l'altra � esaurita
        if (ll_key == ll_key1){
