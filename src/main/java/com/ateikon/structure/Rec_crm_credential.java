package com.ateikon.structure;

import java.sql.Timestamp;



public class Rec_crm_credential extends Object {

    public Rec_crm_credential() {

        super();
    }

    public Rec_crm_credential( com.ateikon.common.Crm_credential obj_common_from
                       , com.ateikon.common.Crm_credential obj_common_to   ) {

        super();

        if (obj_common_from !=null){
            this.obj_common_from = obj_common_from;
        }
        if (obj_common_to != null){
            this.obj_common_to = obj_common_to;
        }
    }
    public Rec_crm_credential( com.ateikon.common.Crm_credential obj_common_to   ) {

        super();

        if (obj_common_to != null){
            this.obj_common_to = obj_common_to;
        }
    }
    public com.ateikon.common.Crm_credential obj_common_from = null;
    public com.ateikon.common.Crm_credential obj_common_to   = null;


    public String seStesso = "Rec_crm_credential";

    public String rc          = "";
    public String message     = "";



    /****
        Properties 
    **/ 


    public long       tkcredential = 0; 
    public String     keycode = ""; 
    public Timestamp  dtival = null; 
    public Timestamp  dtfval = null; 
    public String     cdazie = ""; 
    public String     cddipa = ""; 
    public String     profil = ""; 
    public Timestamp  dtinse = null; 
    public Timestamp  dtulag = null; 
    public long       tkutente = 0; 






    /****
        init (): Inizializza la struttrra  
    **/ 

    public void init() throws Exception{ 

        this.tkcredential = 0; 
        this.keycode = ""; 
        this.dtival = null; 
        this.dtfval = null; 
        this.cdazie = ""; 
        this.cddipa = ""; 
        this.profil = ""; 
        this.dtinse = null; 
        this.dtulag = null; 
        this.tkutente = 0; 

    }




    /****
        copy_from()
    **/ 

    public void copy_from(Rec_crm_credential astr) throws Exception{ 

        this.tkcredential = astr.tkcredential;
        this.keycode = astr.keycode;
        if (astr.dtival == null){
           this.dtival = null;
        }else {
           this.dtival = (Timestamp ) astr.dtival.clone();
        }
        if (astr.dtfval == null){
           this.dtfval = null;
        }else {
           this.dtfval = (Timestamp ) astr.dtfval.clone();
        }
        this.cdazie = astr.cdazie;
        this.cddipa = astr.cddipa;
        this.profil = astr.profil;
        if (astr.dtinse == null){
           this.dtinse = null;
        }else {
           this.dtinse = (Timestamp ) astr.dtinse.clone();
        }
        if (astr.dtulag == null){
           this.dtulag = null;
        }else {
           this.dtulag = (Timestamp ) astr.dtulag.clone();
        }
        this.tkutente = astr.tkutente;

    }





    /****
        setResultSet: crm_credential
    **/ 


    public int setResultSet ( java.sql.ResultSet rs) throws Exception {


        this.init();

        if (rs == null) return 0;
        if (rs.getObject("tkcredential")!=null){
            this.tkcredential = rs.getLong      ("tkcredential"); 
            if (obj_common_to   != null) obj_common_to.null_tkcredential = false;
            if (obj_common_from != null) obj_common_from.null_tkcredential = false;
        }else {
            if (obj_common_to   != null) obj_common_to.null_tkcredential = true;
            if (obj_common_from != null) obj_common_from.null_tkcredential = true;
        }
        if (rs.getObject("keycode")!=null) this.keycode = rs.getString    ("keycode"); 
        if (rs.getObject("dtival")!=null) this.dtival = rs.getTimestamp ("dtival"); 
        if (rs.getObject("dtfval")!=null) this.dtfval = rs.getTimestamp ("dtfval"); 
        if (rs.getObject("cdazie")!=null) this.cdazie = rs.getString    ("cdazie"); 
        if (rs.getObject("cddipa")!=null) this.cddipa = rs.getString    ("cddipa"); 
        if (rs.getObject("profil")!=null) this.profil = rs.getString    ("profil"); 
        if (rs.getObject("dtinse")!=null) this.dtinse = rs.getTimestamp ("dtinse"); 
        if (rs.getObject("dtulag")!=null) this.dtulag = rs.getTimestamp ("dtulag"); 
        if (rs.getObject("tkutente")!=null){
            this.tkutente = rs.getLong      ("tkutente"); 
            if (obj_common_to   != null) obj_common_to.null_tkutente = false;
            if (obj_common_from != null) obj_common_from.null_tkutente = false;
        }else {
            if (obj_common_to   != null) obj_common_to.null_tkutente = true;
            if (obj_common_from != null) obj_common_from.null_tkutente = true;
        }

        return 1;
    }




    /****
        setResultSet_key: crm_credential
    **/ 


    public int setResultSet_key ( java.sql.ResultSet rs) throws Exception {


        this.init();

        if (rs == null) return 0;
        if (rs.getObject("tkcredential")!=null){
            this.tkcredential = rs.getLong      ("tkcredential"); 
            if (obj_common_to   != null) obj_common_to.null_tkcredential = false;
            if (obj_common_from != null) obj_common_from.null_tkcredential = false;
        }else {
            if (obj_common_to   != null) obj_common_to.null_tkcredential = true;
            if (obj_common_from != null) obj_common_from.null_tkcredential = true;
        }

        return 1;
    }




    /****
        trim(): crm_credential
    **/ 


    public void trim( ) throws Exception {

        this.keycode = this.keycode.trim(); 
        this.cdazie = this.cdazie.trim(); 
        this.cddipa = this.cddipa.trim(); 
        this.profil = this.profil.trim(); 

    }



    public void rtrim( ) throws Exception {

        this.keycode = of_rtrim(this.keycode); 
        this.cdazie = of_rtrim(this.cdazie); 
        this.cddipa = of_rtrim(this.cddipa); 
        this.profil = of_rtrim(this.profil); 

    }


    public String of_rtrim( String as_ ) throws Exception {

        String ls_pref = "";
        int    pos     = 0; 
        int    len     = as_.length();

        while(pos < len && as_.charAt(pos) == ' '){
            ls_pref += " "; 
            pos += 1;
        }
        as_ = ls_pref + as_.trim();

        return as_;
    }




    /****
        ctrl_mindate(): crm_credential
    **/ 


    public void ctrl_mindate( ) throws Exception {

        int li_sa = 0; 

        li_sa = 0; 
        if (this.dtival != null){ 
            li_sa = this.dtival.getYear()+1900; 
            if (li_sa < 1900){ 
                this.dtival.setYear(0); 
                this.dtival.setMonth(0); 
                this.dtival.setDate(1); 
                this.dtival.setHours(0); 
                this.dtival.setMinutes(0); 
                this.dtival.setSeconds(0); 
                this.dtival.setNanos(0); 
            } 
            this.dtival.setNanos(0); 
            this.dtival.toString(); 
        } 
        li_sa = 0; 
        if (this.dtfval != null){ 
            li_sa = this.dtfval.getYear()+1900; 
            if (li_sa < 1900){ 
                this.dtfval.setYear(0); 
                this.dtfval.setMonth(0); 
                this.dtfval.setDate(1); 
                this.dtfval.setHours(0); 
                this.dtfval.setMinutes(0); 
                this.dtfval.setSeconds(0); 
                this.dtfval.setNanos(0); 
            } 
            this.dtfval.setNanos(0); 
            this.dtfval.toString(); 
        } 
        li_sa = 0; 
        if (this.dtinse != null){ 
            li_sa = this.dtinse.getYear()+1900; 
            if (li_sa < 1900){ 
                this.dtinse.setYear(0); 
                this.dtinse.setMonth(0); 
                this.dtinse.setDate(1); 
                this.dtinse.setHours(0); 
                this.dtinse.setMinutes(0); 
                this.dtinse.setSeconds(0); 
                this.dtinse.setNanos(0); 
            } 
            this.dtinse.setNanos(0); 
            this.dtinse.toString(); 
        } 
        li_sa = 0; 
        if (this.dtulag != null){ 
            li_sa = this.dtulag.getYear()+1900; 
            if (li_sa < 1900){ 
                this.dtulag.setYear(0); 
                this.dtulag.setMonth(0); 
                this.dtulag.setDate(1); 
                this.dtulag.setHours(0); 
                this.dtulag.setMinutes(0); 
                this.dtulag.setSeconds(0); 
                this.dtulag.setNanos(0); 
            } 
            this.dtulag.setNanos(0); 
            this.dtulag.toString(); 
        } 

    }




    /****
        requestValues(request):   
    **/ 

    public void requestValues(javax.servlet.http.HttpServletRequest request) throws Exception{ 

        if (request.getParameterValues("tkcredential")!=null) arr_tkcredential = request.getParameterValues("tkcredential");
        if (request.getParameterValues("keycode")!=null) arr_keycode = request.getParameterValues("keycode");
        if (request.getParameterValues("dtival")!=null) arr_dtival = request.getParameterValues("dtival");
        if (request.getParameterValues("dtfval")!=null) arr_dtfval = request.getParameterValues("dtfval");
        if (request.getParameterValues("cdazie")!=null) arr_cdazie = request.getParameterValues("cdazie");
        if (request.getParameterValues("cddipa")!=null) arr_cddipa = request.getParameterValues("cddipa");
        if (request.getParameterValues("profil")!=null) arr_profil = request.getParameterValues("profil");
        if (request.getParameterValues("dtinse")!=null) arr_dtinse = request.getParameterValues("dtinse");
        if (request.getParameterValues("dtulag")!=null) arr_dtulag = request.getParameterValues("dtulag");
        if (request.getParameterValues("tkutente")!=null) arr_tkutente = request.getParameterValues("tkutente");

        return ;
    }

    public String[] arr_tkcredential = null; 
    public String[] arr_keycode = null; 
    public String[] arr_dtival = null; 
    public String[] arr_dtfval = null; 
    public String[] arr_cdazie = null; 
    public String[] arr_cddipa = null; 
    public String[] arr_profil = null; 
    public String[] arr_dtinse = null; 
    public String[] arr_dtulag = null; 
    public String[] arr_tkutente = null; 





}

